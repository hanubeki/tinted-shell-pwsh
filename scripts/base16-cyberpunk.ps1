# tinted-shell (base16) Cyberpunk for PowerShell
# scheme made by benjujo

$Env:BASE16_THEME = "cyberpunk"

Write-Host -NoNewline "`e]4;0;rgb:00/00/00`e\" # base00
Write-Host -NoNewline "`e]4;1;rgb:ff/00/00`e\" # base08
Write-Host -NoNewline "`e]4;2;rgb:61/ce/3c`e\" # base0B
Write-Host -NoNewline "`e]4;3;rgb:ff/ff/00`e\" # base0A
Write-Host -NoNewline "`e]4;4;rgb:4c/83/ff`e\" # base0D
Write-Host -NoNewline "`e]4;5;rgb:ff/14/93`e\" # base0E
Write-Host -NoNewline "`e]4;6;rgb:93/e0/e3`e\" # base0C
Write-Host -NoNewline "`e]4;7;rgb:dc/dc/cc`e\" # base05

Write-Host -NoNewline "`e]4;8;rgb:4f/4f/4f`e\" # base03
Write-Host -NoNewline "`e]4;9;rgb:ff/00/00`e\" # base08
Write-Host -NoNewline "`e]4;10;rgb:61/ce/3c`e\" # base0B
Write-Host -NoNewline "`e]4;11;rgb:ff/ff/00`e\" # base0A
Write-Host -NoNewline "`e]4;12;rgb:4c/83/ff`e\" # base0D
Write-Host -NoNewline "`e]4;13;rgb:ff/14/93`e\" # base0E
Write-Host -NoNewline "`e]4;14;rgb:93/e0/e3`e\" # base0C
Write-Host -NoNewline "`e]4;15;rgb:ff/ff/ff`e\" # base07

Write-Host -NoNewline "`e]4;16;rgb:ff/a5/00`e\" # base09
Write-Host -NoNewline "`e]4;17;rgb:ff/8c/00`e\" # base0F
Write-Host -NoNewline "`e]4;18;rgb:2b/2b/2b`e\" # base01
Write-Host -NoNewline "`e]4;19;rgb:38/38/38`e\" # base02
Write-Host -NoNewline "`e]4;20;rgb:8b/89/89`e\" # base04
Write-Host -NoNewline "`e]4;21;rgb:f8/f8/f8`e\" # base06

Write-Host -NoNewline "`e]10;rgb:dc/dc/cc`e\" # base05

if ($Env:BASE16_SHELL_SET_BACKGROUND -ne "false") {
    Write-Host -NoNewline "`e]11;rgb:00/00/00`e\" # base00
}

Write-Host -NoNewline "`e]12;rgb:dc/dc/cc`e\" # base05

if ($(Test-Path "Env:TINTED_SHELL_ENABLE_BASE16_VARS") -or $(Test-Path "Env:BASE16_SHELL_ENABLE_VARS")) {
    $Env:BASE16_COLOR_00_HEX = "000000"
    $Env:BASE16_COLOR_01_HEX = "2b2b2b"
    $Env:BASE16_COLOR_02_HEX = "383838"
    $Env:BASE16_COLOR_03_HEX = "4f4f4f"
    $Env:BASE16_COLOR_04_HEX = "8b8989"
    $Env:BASE16_COLOR_05_HEX = "dcdccc"
    $Env:BASE16_COLOR_06_HEX = "f8f8f8"
    $Env:BASE16_COLOR_07_HEX = "ffffff"
    $Env:BASE16_COLOR_08_HEX = "ff0000"
    $Env:BASE16_COLOR_09_HEX = "ffa500"
    $Env:BASE16_COLOR_0A_HEX = "ffff00"
    $Env:BASE16_COLOR_0B_HEX = "61ce3c"
    $Env:BASE16_COLOR_0C_HEX = "93e0e3"
    $Env:BASE16_COLOR_0D_HEX = "4c83ff"
    $Env:BASE16_COLOR_0E_HEX = "ff1493"
    $Env:BASE16_COLOR_0F_HEX = "ff8c00"
}
